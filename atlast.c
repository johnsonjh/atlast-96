 ...
 * the compiler call line), we include the file "atlcfig.h", which may then
 * define INDIVIDUALLY and select the subpackages needed for its application.
 */
...
#define MEMORY_MODEL_ERROR(condition) \
...
#ifdef ATLAST_64BIT                  /* 64-bit?                            */
  atl_int  atl_heaplen   = 2000;     /* 64-bit Heap length                 */
#else                                /* 32-bit?                            */
  atl_int  atl_heaplen   = 1000;     /* 32-bit Heap length                 */
...
Exported stackitem * heapbot;         /* Bottom of heap          */
...
Exported dictword *  createword  = NULL;      /* Addr of word pending create */
...
static stackitem s_exit, s_lit, s_flit, s_strlit, s_dotparen, s_qbranch,
                 s_branch, s_xdo, s_xqdo, s_xloop, s_pxloop, s_abortq;
...
/*
 * UCASE
 * Force letters in string to upper case.
 */
...
/*
 * LOOKUP
 * Look up token in the dictionary.
 */
...
/*
 * ATL_FGETSP
 * Portable database version of FGETS.
 *
 * This reads the next line into a buffer a la fgets().  A line is delimited
 * by either a carriage return or a line feed, optionally followed by the
 * other character of the pair.  The string is always null terminated, and
 * limited to the length specified - 1 (excess characters on the line are
 * discarded). The string is returned, or NULL if end of file is encountered
 * and no characters were stored.  No end of line character is stored in the
 * string buffer.
 */

  Exported char *
  atl_fgetsp(s, n, stream)
  char *s;
  int    n;
  FILE * stream;
  {
 ...
 */
  ...
  atl_memstat()
  {
    ...
    V printf(fmt,
             "Stack", ((long)( stk - stack )), ((long)( stackmax - stack )),
             atl_stklen, ( 100L * ( stk - stack )) / atl_stklen);
    V printf(fmt,
             "Return stack", ((long)( rstk - rstack )),
             ((long)( rstackmax - rstack )), atl_rstklen,
...
/*
 * ENTER
 * Enter word in dictionary.
 *
 * Given token for word's name and initial values for it's attributes,
 * returns the newly-allocated dictionary item.
 */
...
prim
P_divmod()   /* Compute quotient and remainder */
{
  stackitem quot;
...
prim
P_min()      /* Take minimum of stack top */
{
  Sl(2);
  S1 = min(S1, S0);
  Pop;
...
prim
P_max()      /* Take maximum of stack top */
{
  Sl(2);
  S1 = max(S1, S0);
  Pop;
...
prim
P_abs()      /* Take absolute value of top of stack */
{
  Sl(1);
  S0 = abs(S0);
}
...
prim
P_geq()      /* Test greater than or equal */
{
  Sl(2);
  S1 = ( S1 >= S0 ) ? Truth : Falsity;
  Pop;
...
prim
P_shift()    /* Shift:  value nbits -- value */
{
  Sl(1);
  S1 = ( S0 < 0 ) ? (((unsigned long)S1 )
      >> ( -S0 )) : (((unsigned long)S1 )
      << S0 );
  Pop;
}
  ...
  prim
  P_2div()   /* Divide by two */
  {
    Sl(1);
    S0 /= 2;
  }
#endif /* ifdef SHORTCUTA */

#ifdef SHORTCUTC
  prim
  P_0equal()    /* Equal to zero ? */
  {
    Sl(1);
  ...
  prim
  P_0lss()      /* Less than zero ? */
  {
    Sl(1);
    S0 = ( S0 < 0 ) ? Truth : Falsity;
  }
#endif /* ifdef SHORTCUTC */

/* Storage allocation (heap) primitives */

prim
P_here()        /* Push current heap address */
{
  So(1);
  Push = (stackitem)hptr;
}
...
prim
P_bang()        /* Store value into address */
{
  Sl(2);
  Hpc(S0);
  *((stackitem *)S0 ) = S1;
...
prim
P_plusbang()    /* Add value at specified address */
{
  Sl(2);
  Hpc(S0);
  *((stackitem *)S0 ) += S1;
...
prim
P_comma()       /* Store one item on heap */
{
  Sl(1);
  Ho(1);
  Hstore = S0;
...
prim
P_cbang()       /* Store byte value into address */
{
  Sl(2);
  Hpc(S0);
  *((unsigned char *)S0 ) = S1;
...
prim
P_cat()         /* Fetch byte value from address */
{
  Sl(1);
  Hpc(S0);
  S0 = *((unsigned char *)S0 );
...
prim
P_ccomma()      /* Store one byte on heap */
{
  unsigned char *chp;
...
prim
P_cequal()      /* Align heap pointer after storing */
{               /*   a series of bytes.             */
...
prim
P_var()         /* Push body address of current word */
{
  So(1);
...
prim
P_forget()      /* Forget word */
{
  forgetpend = True; /* Mark forget pending */
}

prim
P_variable()    /* Declare variable */
{
...
#ifdef ARRAY
  prim
  P_arraysub()  /* Array subscript calculation */
  {             /*   sub1 sub2 ... subn -- addr */
...
#ifdef STRING
  prim
  P_strlit()    /* Push address of string literal */
  {
    So(1);
...
# endif /* ifdef TRACE */
  ...
  prim
  P_strcpy()    /* Copy string to address on stack */
  {
    Sl(2);
    Hpc(S0);
    Hpc(S1);
  ...
  prim
  P_strcat()    /* Append string to address on stack */
  {
    Sl(2);
    Hpc(S0);
    Hpc(S1);
  ...
  prim
  P_strlen()    /* Take length of string on stack top */
  {
    Sl(1);
    Hpc(S0);
    S0 = strlen((char *)S0);
  ...
  prim
  P_strcmp()    /* Compare top two strings on stack */
  {
    int i;
  ...
  prim
  P_strchar()   /* Find character in string */
  {
    Sl(2);
    Hpc(S0);
    Hpc(S1);
  ...
  prim
  P_substr()    /* Extract and store substring */
  ...
  prim
  P_strform()   /* Format integer using sprintf() */
  {             /*   value "%ld" str -- */
    Sl(2);
    Hpc(S0);
    Hpc(S1);
...
# ifdef REAL
    prim
    P_fstrform() /* Format real using sprintf() */
    {            /*   rvalue "%6.2f" str -- */
#  ifdef ATLAST_64BIT
        Sl(2 + Realsize);
#  else
...
#ifdef REAL
  prim
  P_flit()      /* Push floating point literal */
  {
    int i;
...
# endif /* ifdef TRACE */
  ...
  prim
  P_fplus()     /* Add floating point numbers */
  {
    Sl(2 * Realsize);
    SREAL1(REAL1 + REAL0);
    Realpop;
  ...
  prim
  P_fminus()    /* Subtract floating point numbers */
  {
    Sl(2 * Realsize);
    SREAL1(REAL1 - REAL0);
    Realpop;
  ...
  prim
  P_ftimes()    /* Multiply floating point numbers */
  {
    Sl(2 * Realsize);
    SREAL1(REAL1 * REAL0);
    Realpop;
  ...
  prim
  P_fdiv()      /* Divide floating point numbers */
  {
    Sl(2 * Realsize);
# ifndef NOMEMCHECK
      if (REAL0 == 0.0)
...
# endif /* NOMEMCHECK */
  ...
  prim
  P_fmin()      /* Minimum of top two floats */
  {
    Sl(2 * Realsize);
  ...
  prim
  P_fmax()      /* Maximum of top two floats */
  {
    Sl(2 * Realsize);
  ...
  prim
  P_fabs()      /* Absolute value of top of stack */
  {
    Sl(Realsize);
    SREAL0(abs(REAL0));
  }
  ...
  prim
  P_fequal()    /* Test equality of top of stack */
  {
    stackitem t;
  ...
  prim
  P_funequal()  /* Test inequality of top of stack */
  {
    stackitem t;
  ...
  prim
  P_fgeq()      /* Test greater than or equal */
  {
    stackitem t;
  ...
  prim
  P_fleq()      /* Test less than or equal */
  {
    stackitem t;
  ...
  prim
  P_fdot()      /* Print floating point top of stack */
  {
    Sl(Realsize);
    V printf("%g ", REAL0);
    Realpop;
  ...
  prim
  P_float()     /* Convert integer to floating */
  {
    atl_real r;
  ...
  prim
  P_fix()       /* Convert floating to integer */
  {
    stackitem i;
 ...
 */
...
#ifdef CONIO
  prim
  P_dot()       /* Print top of stack, pop it */
  {
    Sl(1);
  ...
  prim
  P_question()  /* Print value at address */
  {
    Sl(1);
    Hpc(S0);
  ...
  prim
  P_cr()        /* Carriage return */
  {
    V printf("\r\n"); /* Actually CRLF */
  }

  prim
  P_dots()      /* Print entire contents of stack */
  {
    stackitem *tsp;
  ...
  prim
  P_dotquote()  /* Print literal string that follows */
  {
    Compiling;
  ...
  prim
  P_dotparen()  /* Print literal string that follows */
  {
    if (ip == NULL)
      {                    /* If interpreting                   */
  ...
  prim
  P_type()      /* Print string pointed to by stack */
  {
    Sl(1);
    Hpc(S0);
    V printf("%s", (char *)S0);
...
# endif /* ifdef Keyhit */
  ...
  prim
  P_fopen()     /* Open file: fname fmodes fd -- flag */
  {
    FILE *     fd;
    stackitem  stat;
  ...
  prim
  P_fdelete()   /* Delete file: fname -- flag */
  {
    Sl(1);
    Hpc(S0);
  ...
  prim
  P_fgetline()  /* Get line: fd string -- flag */
  {
    Sl(2);
    Hpc(S0);
    Isfile(S1);
  ...
  prim
  P_fputline()  /* Put line: string fd -- flag */
  {
    Sl(2);
    Hpc(S1);
    Isfile(S0);
  ...
  prim
  P_fread()     /* File read: fd len buf -- length */
  {
    Sl(3);
    Hpc(S0);
    Isfile(S2);
  ...
  prim
  P_fwrite()    /* File write: len buf fd -- length */
  {
    Sl(3);
    Hpc(S1);
    Isfile(S0);
  ...
  prim
  P_fgetc()     /* File get character: fd -- char */
  {
    Sl(1);
    Isfile(S0);
    Isopen(S0);
  ...
  prim
  P_fputc()     /* File put character: char fd -- stat */
  {
    Sl(2);
    Isfile(S0);
    Isopen(S0);
  ...
  prim
  P_ftell()     /* Return file position:  fd -- pos */
  {
    Sl(1);
    Isfile(S0);
    Isopen(S0);
  ...
  prim
  P_fseek()     /* Seek file:  offset base fd -- */
  {
    Sl(3);
    Isfile(S0);
    Isopen(S0);
  ...
  prim
  P_fload()     /* Load source file:  fd -- evalstat */
  {
    int    estat;
    FILE * fd;
  ...
  { /* string -- status */
     ...
     * If we ended the file in comment-ignore mode, set the runaway
     * comment error status and unwind the file.
     */
...
prim
P_swap()        /* Exchange two top items on stack */
{
  stackitem t;
...
prim
P_over()        /* Push copy of next to top of stack */
{
  stackitem s;
...
prim
P_pick()        /* Copy indexed item from stack */
{
  Sl(2);
  S0 = stk[-( 2 + S0 )];
}
...
prim
P_rot()         /* Rotate 3 top stack items */
{
  stackitem t;
...
prim
P_minusrot()    /* Reverse rotate 3 top stack items */
{
  stackitem t;
...
prim
P_roll()        /* Rotate N top stack items */
{
  stackitem i, j, t;
...
prim
P_tor()         /* Transfer stack top to return stack */
{
  Rso(1);
  Sl(1);
  Rpush = (rstackitem)S0;
...
prim
P_rfrom()       /* Transfer return stack top to stack */
{
  Rsl(1);
  So(1);
  Push = (stackitem)R0;
...
prim
P_rfetch()      /* Fetch top item from return stack */
{
  Rsl(1);
  So(1);
  Push = (stackitem)R0;
...
/* Double stack manipulation items */
  ...
  prim
  P_2dup()      /* Duplicate stack top doubleword */
  {
    stackitem s;
  ...
  prim
  P_2drop()     /* Drop top two items from stack */
  {
    Sl(2);
    stk -= 2;
  }
  ...
  prim
  P_2swap()     /* Swap top two double items on stack */
  {
    stackitem t;
  ...
  prim
  P_2over()     /* Extract second pair from stack */
  {
    stackitem s;
  ...
  prim
  P_2rot()      /* Move third pair to top of stack */
  {
    stackitem t1, t2;
  ...
  prim
  P_2variable() /* Declare double variable */
  {
    P_create(); /* Create dictionary item */
    Ho(2);
    Hstore  = 0; /* Initial value = 0... */
  ...
  prim
  P_2con()      /* Push double value in body */
  {
    So(2);
    Push  = *(((stackitem *)curword ) + Dictwordl );
  ...
  prim
  P_2constant() /* Declare double word constant */
  {
    Sl(1);
    P_create();                  /* Create dictionary item    */
  ...
  prim
  P_2bang()     /* Store double value into address */
  {
    stackitem *sp;
  ...
  prim
  P_2at()       /* Fetch double value from address */
  {
    stackitem *sp;
...
prim
P_dolit()       /* Push instruction stream literal */
{
  So(1);
#ifdef TRACE
    if (atl_trace)
...
prim
P_exit()        /* Return to top of return stack */
{
  Rsl(1);
#ifdef WALKBACK
...
prim
P_branch()      /* Jump to in-line address */
{
  ip += ( stackitem ) * ip; /* Jump addresses are IP-relative */
}

prim
P_qbranch()     /* Conditional branch to in-line addr */
{
  Sl(1);
...
P_xdo()         /* Execute DO */
{
  ...
  Rpush  = (rstackitem)S0;             /* Iteration variable initial value */
                                       /*   to return stack                */
  stk    -= 2;
}
...
P_xqdo()        /* Execute ?DO */
{
  ...
  if (S0 == S1)
  ...
  else
    {
      Rso(3);
      Rpush  = ip + (( stackitem ) * ip ); /* Push exit address from loop  */
      ip++;                                /* Incr. past exit address word */
      Rpush  = (rstackitem)S1;             /* Push loop limit on ret stack */
      Rpush  = (rstackitem)S0;             /* Iteration variable initial   */
                                           /*   value to return stack      */
    }
...
prim
P_i()           /* Obtain innermost loop index */
{
  Rsl(3);
  So(1);
...
prim
P_j()           /* Obtain next-innermost loop index */
{
  Rsl(6);
  So(1);
...
prim
P_abort()       /* Abort, clearing data stack */
{
...
prim
P_abortq()      /* Abort, printing message */
{
  if (state)
    {
      stringlit = True;     /* Set string literal expected */
...
prim
P_immediate()   /* Mark most recent word immediate */
{
  dict->wname[0] |= IMMEDIATE;
}

prim
P_lbrack()      /* Set interpret state */
{
  Compiling;
  state = Falsity;
}
...
prim
P_rbrack()      /* Restore compile state */
{
  state = Truth;
}

Exported void
P_dodoes()      /* Execute indirect call on method */
{
  Rso(1);
  So(1);
  Rpush       = ip; /* Push instruction pointer */
  ...
  /*
   * Push the address of this word's body
   * as the argument to the DOES> clause.
   */
...
prim
P_does()        /* Specify method for word */
{
  ...
  if (createword != NULL)
    {
      ...
      hptr++;                              /* Expand alloc'd length of word  */
      *sp++              = (stackitem)ip;  /* Store DOES> clause addr before */
                                           /*   word's definition structure. */
      createword         = (dictword *)sp; /* Move word defn down by 1 item  */
      createword->wcode  = P_dodoes;       /* Set code field to indirect jmp */
...
P_semicolon()   /* End compilation */
{
  ...
  /*
   * We wait until now to plug the P_nest code so that
   * it will be present only in completed definitions.
   */
...
prim
P_tick()        /* Take address of next word */
{
  int i;

  /*
   * Try to get next symbol from the input stream.
   * If we can't, and we're executing a compiled word,
  ...
  if (i != TokNull)
  ...
  else
    {
      ...
      /*
       * O.K., there was nothing in the input stream.  Set the tickpend
       * flag to cause the compilation address of the next token to be
       * pushed when it's supplied on a subsequent input line.
       */
...
prim
P_bracktick()   /* Compile in-line code address */
{
  Compiling;
...
prim
P_execute()     /* Execute word pointed to by stack */
{
  dictword *wp;
...
prim
P_body()        /* Get body address for word */
{
  Sl(1);
...
#ifdef DEFFIELDS
  prim
  P_find()      /* Look up word in dictionary */
  {
    dictword *dw;
...
# define DfOff(fld)  (((char *)&( dict->fld )) - ((char *)dict ))
  ...
  prim
  P_toname()    /* Find name field from compile addr */
  {
    Sl(1);
    S0 += DfOff(wname);
  }
  ...
  prim
  P_tolink()    /* Find link field from compile addr */
  {
    if (DfOff(wnext) != 0)
      {
  ...
  prim
  P_frombody()  /* Get compile address from body */
  {
    Sl(1);
    S0 -= Dictwordl * sizeof ( stackitem );
  }
  ...
  prim
  P_fromname()  /* Get compile address from name */
  {
    Sl(1);
    S0 -= DfOff(wname);
  }
  ...
  prim
  P_fromlink()  /* Get compile address from link */
  {
    if (DfOff(wnext) != 0)
      {
        V printf("\nLINK> Foulup--wnext is not at zero!\n");
...
# define DfTran(from, to) \
    (((char *)&( dict->to )) - ((char *)&( dict->from )))

  prim
  P_nametolink() /* Get from name field to link */
  {
    char *from, *to;
  ...
  prim
  P_linktoname() /* Get from link field to name */
  {
    char *from, *to;
  ...
  prim
  P_fetchname() /* Copy word name to string buffer */
  {
    Sl(2); /* nfa string -- */
    Hpc(S0);
    Hpc(S1);
    ...
    /*
     * Since the name buffers aren't in the system heap, but rather are
     * separately allocated with alloc(), we can't check the name pointer
     * references.  But, hey, if the user's futzing with word dictionary
     * items on the heap in the first place, there's a billion other ways
     * to bring us down at his command.
     */
  ...
  prim
  P_storename() /* Store string buffer in word name */
  {
    char   tflags;
    char * cp;
...
#ifdef TRACE
  prim
  P_trace()     /* Set or clear tracing of execution */
  {
    Sl(1);
...
#ifdef WALKBACK
  prim
  P_walkback()  /* Set or clear error walkback */
  {
    Sl(1);
...
# endif /* ifdef Keyhit */
  ...
  prim
  P_wordsunused() /* List words not used by program */
  {
    dictword *dw = dict;
...
# endif /* ifdef Keyhit */
...
#ifdef COMPILERW
  prim
  P_brackcompile() /* Force compilation of immediate word */
  {
    Compiling;
  ...
  prim
  P_literal()   /* Compile top of stack as literal */
  {
    Compiling;
    Sl(1);
    Ho(2);
  ...
  prim
  P_compile()   /* Compile address of next inline word */
  {
    Compiling;
    Ho(1);
  ...
  prim
  P_backmark()  /* Mark backward backpatch address */
  {
    Compiling;
    So(1);
  ...
  prim
  P_backresolve() /* Emit backward jump offset */
  {
    stackitem offset;
  ...
  prim
  P_fwdmark()   /* Mark forward backpatch address */
  {
    Compiling;
    Ho(1);
    Push    = (stackitem)hptr; /* Push heap address onto stack */
  ...
  prim
  P_fwdresolve() /* Emit forward jump offset */
  {
    stackitem offset;
...
#endif /* ifdef COMPILERW */

/*
 * Table of primitive words
 */

static struct primfcn primt[] = { { "0+",           P_plus         },
                                  { "0-",           P_minus        },
                                  { "0*",           P_times        },
                                  { "0/",           P_div          },
                                  { "0MOD",         P_mod          },
...
/*
 * TROUBLE
 * Common handler for serious errors.
 */
...
/*
 * ATL_ERROR
 * Handle error detected by user-defined primitive.
 */
...
/*
 * RSTAKOVER
 * Recover from return stack overflow.
 */
...
/*
 * RSTAKUNDER
 * Recover from return stack underflow.
 */
...
/*
 * HEAPOVER
 * Recover from heap overflow.
 *
 * Note that a heap overflow does NOT wipe the heap; it's up to
 * the user to do this manually with FORGET or some such.
 */
...
/*
 * BADPOINTER
 * Abort if bad pointer reference detected.
 */
...
/*
 * NOTCOMP
 * Compiler word used outside definition.
 */
...
/*
 * EXWORD
 * Execute a word (and any sub-words it may invoke).
 */

static void exword(wp) dictword *wp;
{
  curword = wp;
#ifdef TRACE
    if (atl_trace)
...
/*
 * ATL_INIT
 * Initialise the ATLAST system.
 *
 * The dynamic storage areas are allocated unless the caller has preallocated
 * buffers for them and stored the addresses into the respective pointers.
 * In either case, the storage management pointers are initialised to the
 * correct addresses.  If the caller preallocates the buffers, it's up to him
 * to ensure that the length allocated agrees with the lengths given by the
 * atl_... cells.
 */
...
atl_init()
{
  ...
  /*
   * If any of the following lines of code produce errors, you are
   * probably trying to build on an architecture which violates the
   * fundamental requirement of 64-bit ATLAST that stack items, pointers,
   * and real numbers are all 64-bit values, stored as eight 8-bit bytes.
   * If this is the case and you can't fix the problem by changing compiler
   * memory model options, it's best to revert to a 32-bit compilation.
   */
  ...
  if (dict == NULL)
    {
      ...
      /*
       * Look up compiler-referenced words in
       * the new dictionary and save their
       * compile addresses in static variables.
       */
...
#undef Cconst
      ...
      if (stack == NULL)
        { /* Allocate stack if needed */
          stack = (stackitem *)alloc(
            ((unsigned int)atl_stklen )
            * sizeof ( stackitem ));
        }
      ...
      if (rstack == NULL)
        { /* Allocate return stack if needed */
          rstack = (dictword ***)alloc(
            ((unsigned int)atl_rstklen )
            * sizeof ( dictword * * ));
        }
      ...
      if (heap == NULL)
        {
           ...
           * of the heap, which permits us to pointer-check pointers
           * into them as within the heap extents.  Hence, the size
           * of the buffer we acquire for the heap is the sum of the
           * heap and temporary string requests.
           */
          ...
          /*
           * Force length of temporary strings
           * to even number of stackitems.
           */
          ...
          cp   = alloc(
            (((unsigned int)atl_heaplen ) * sizeof ( stackitem ))
      ...
      /*
       * The system state word is kept in the first
       * word of the heap so that pointer checking
       * doesn't bounce references to it. When creating
       * the heap, we preallocate this word and initialise
      ...
      /*
       * Now that dynamic memory is up and
       * running, allocate constants and
       ...
       */
...
#ifdef FILEIO
        {
          ...
          for (i = 0; i < ELEMENTS(stdfiles); i++)
            {
              if (( dw = atl_vardef(stdfiles[i].sfn, 2 * sizeof ( stackitem )))
                  != NULL)
                {
 ...
 *
 * Returns its word item if found or NULL
...
/*
 * ATL_BODY
 * Returns the address of the body of a word, given its dictionary entry.
 */
...
/*
 * ATL_EXEC
 * Execute a word, given its dictionary address.
 *
 * The evaluation status for that word's execution is returned.
 * The in-progress evaluation status is preserved.
 */
 ...
 *
 * Called with the word's name and the number of bytes of storage to
 * allocate for its body.  All words defined with atl_vardef() have
 * the standard variable action of pushing their body address on the
 * stack when invoked.  Returns the dictionary item for the new word,
 * or NULL if the heap overflows.
 */

dictword *
atl_vardef(name, size)
char *name;
int size;
{
  dictword * di;
  int        isize =
      ( size + ( sizeof ( stackitem ) - 1 )) / sizeof ( stackitem );
...
/*
 * ATL_MARK
 * Mark current state of the system.
 */

void atl_mark(mp) atl_statemark *mp;
{
...
/*
 * ATL_UNWIND
 * Restore system state to previously saved state.
 */

void atl_unwind(mp) atl_statemark *mp;
{
  ...
  /*
   * To unwind the dictionary, we can't just reset the pointer, we must
   * walk back through the chain and release all the name buffers attached
   * to the items allocated after the mark was made.
   */
...
/*
 * ATL_BREAK
 * Asynchronously interrupt execution.
 *
...
FILE *fp;
{
  ...
  /*
   * If there were no other errors, check for a runaway comment.
   * If we ended the file in comment-ignore mode, set the
   * runaway comment error status and unwind the file.
   */
...
/*
 * ATL_PROLOGUE
 * Recognise and process prologue statement.
 *
 * Returns 1 if the statement was part
 * of the prologue and 0 otherwise.
 */
  ...
  if (strncmp(sp, "\\ *", 3) == 0)
    {
      ...
      for (i = 0; i < ELEMENTS(proname); i++)
        {
          if (strncmp(sp + 3, proname[i].pname, strlen(proname[i].pname))
              == 0)
            {
...
/*
 * ATL_EVAL
 * Evaluate a string containing ATLAST words.
 */
...
#define Memerrs  evalstat
  ...
  /*
   * If automatic prologue processing is configured and we haven't
   * yet initialised, check if this is a prologue statement.  If so,
   * execute it.  Otherwise automatically initialise with the memory
   * specifications currently operative.
   */
              ...
              if (( di = lookup(tokbuf)) != NULL)
                {
                   ...
                   * Pass 1.
                   *
                   * Rip through the dictionary to make sure
                   ...
                   * Pass 2.
                   *
                   * Walk back through the dictionary
                   * items until we encounter the target
                   * of the FORGET.  Release each item's
                   * name buffer and de-chain it from the
                   * dictionary list.
                   */
                  ...
                  if (di != NULL)
                    {
                      ...
                      /*
                       * Uhhhh, just one more thing. If this word was
                       * defined with DOES>, there's a link to the method
                       * address hidden before its wnext field.  See if it's
                       * a DOES> by testing the wcode field for P_dodoes and,
                       * if so, back up the heap one more item.
                       */
              ...
              if (di != NULL)
                {
                   ...
                   * Test the state.
                   *
                   * If we're interpreting, execute the word in all cases.
                   * If we're compiling, compile the word unless it is a
                   * compiler word flagged for immediate execution by the
                   * presence of a space as the first character of its name
